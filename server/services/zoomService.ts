...
class ZoomService {
  ...
  public async createMeeting(params: ZoomMeetingParams): Promise<ZoomMeetingResponse> {
    try {
      ...
      const payload: any = {
        ...
        type: params.type || 2, // 2 = scheduled, 1 = instant
        duration: params.duration || 60,
        timezone: 'Asia/Kolkata',
        agenda: params.agenda || 'Trade Nexus Live Video Room',
        settings: {
          host_video: true,
  ...
  public async deleteMeeting(meetingId: string): Promise<boolean> {
    try {
      ...
      const errorText = await res.text();
      console.warn(`[ZoomService] Zoom delete meeting returned status ${res.status}: ${errorText}`);
      return false;
    } catch (error) {
  ...
  public async getMeeting(meetingId: string): Promise<any> {
    try {
      ...
      console.error('[ZoomService] Error fetching meeting details:', error);
      return null;
    }
  }
}
