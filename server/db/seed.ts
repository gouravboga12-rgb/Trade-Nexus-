...
function getTableCount(tableName: string): number {
  try {
    const res = db.prepare(`SELECT COUNT(*) as count FROM ${tableName}`).get() as { count: number };
...
export function seedInitialDataIfEmpty() {
  ...
  if (getTableCount('employee_profiles') === 0) {
    console.log('[SQLite DB] Seeding initial employee profiles...');
  ...
  if (getTableCount('team_members') === 0) {
    console.log('[SQLite DB] Seeding team members roster...');
    // Ensure portal column exists
    try { db.exec("ALTER TABLE team_members ADD COLUMN portal TEXT DEFAULT 'telecaller'"); } catch(_) {}

    const insertTeamMember = db.prepare(`
      INSERT OR REPLACE INTO team_members (id, empCode, name, avatar, role, groupName, phone, attendanceStatus, checkInTime, checkInMethod, dialsToday, goalCalls, connected, interested, salesAchieved, salesTarget, conversionRate, portal)
      VALUES (?, ?, ?, ?, ?, ?, ?, ?, ?, ?, ?, ?, ?, ?, ?, ?, ?, ?)
    `);
    const members = [
      { id: 'tm-tl-1', empCode: 'TNX-TL01', name: 'Team Leader', avatar: 'TL', role: 'Sales Team Leader', groupName: 'HNI Closers', phone: '+91 98450 22222', attendanceStatus: 'PRESENT', checkInTime: '09:00 AM', checkInMethod: 'Face ID Biometric', dialsToday: 15, goalCalls: 40, connected: 12, interested: 4, salesAchieved: 250000, salesTarget: 400000, conversionRate: 25.0, portal: 'team_leader' },
      { id: 'tm-hr-1', empCode: 'TNX-8096', name: 'HR Officer', avatar: 'HR', role: 'HR Operations Officer', groupName: 'HR & People Ops', phone: '+91 98450 80960', attendanceStatus: 'PRESENT', checkInTime: '09:00 AM', checkInMethod: 'Face ID Biometric', dialsToday: 0, goalCalls: 0, connected: 0, interested: 0, salesAchieved: 0, salesTarget: 0, conversionRate: 0, portal: 'hr' },
      { id: 'tm-1', empCode: 'TNX-8275', name: 'Gourav Boga', avatar: 'GB', role: 'Senior SDR Specialist', groupName: 'HNI Closers', phone: '+91 98450 82750', attendanceStatus: 'PRESENT', checkInTime: '09:12 AM', checkInMethod: 'Face ID Biometric', dialsToday: 68, goalCalls: 100, connected: 44, interested: 12, salesAchieved: 145000, salesTarget: 200000, conversionRate: 17.6, portal: 'telecaller' },
      { id: 'tm-2', empCode: 'TNX-8493', name: 'Priya Nair', avatar: 'PN', role: 'Inside Sales Team Leader', groupName: 'HNI Closers', phone: '+91 98450 67890', attendanceStatus: 'PRESENT', checkInTime: '09:05 AM', checkInMethod: 'Face ID Biometric', dialsToday: 82, goalCalls: 100, connected: 58, interested: 16, salesAchieved: 190000, salesTarget: 200000, conversionRate: 19.5, portal: 'team_leader' },
      { id: 'tm-3', empCode: 'TNX-8316', name: 'Zoro Juro', avatar: 'ZJ', role: 'Telecaller Executive', groupName: 'Inbound Qualifiers', phone: '+91 98450 83160', attendanceStatus: 'PRESENT', checkInTime: '09:28 AM', checkInMethod: 'Geo-tagged', dialsToday: 51, goalCalls: 90, connected: 32, interested: 7, salesAchieved: 95000, salesTarget: 180000, conversionRate: 13.7, portal: 'telecaller' },
      { id: 'tm-4', empCode: 'TNX-8442', name: 'Nikhil Bill', avatar: 'NB', role: 'Telecaller Executive', groupName: 'Inbound Qualifiers', phone: '+91 98450 84420', attendanceStatus: 'LATE', checkInTime: '10:15 AM', checkInMethod: 'Face ID Biometric', dialsToday: 39, goalCalls: 90, connected: 21, interested: 5, salesAchieved: 65000, salesTarget: 180000, conversionRate: 12.8, portal: 'telecaller' },
      { id: 'tm-5', empCode: 'TNX-8501', name: 'Rohan Joshi', avatar: 'RJ', role: 'Junior Telecaller', groupName: 'Retention Squad', phone: '+91 98450 99001', attendanceStatus: 'ON_LEAVE', checkInTime: null, checkInMethod: null, dialsToday: 0, goalCalls: 80, connected: 0, interested: 0, salesAchieved: 40000, salesTarget: 150000, conversionRate: 0, portal: 'telecaller' },
      { id: 'tm-6', empCode: 'TNX-8504', name: 'Kavita Menon', avatar: 'KM', role: 'Inside Sales Specialist', groupName: 'Retention Squad', phone: '+91 98450 22334', attendanceStatus: 'PRESENT', checkInTime: '08:58 AM', checkInMethod: 'Face ID Biometric', dialsToday: 74, goalCalls: 100, connected: 49, interested: 11, salesAchieved: 160000, salesTarget: 200000, conversionRate: 14.8, portal: 'telecaller' },
      { id: 'tm-7', empCode: 'TNX-TC01', name: 'Telecaller Executive', avatar: 'TE', role: 'Sales Executive', groupName: 'HNI Closers', phone: '+91 98450 12345', attendanceStatus: 'PRESENT', checkInTime: '09:00 AM', checkInMethod: 'Face ID Biometric', dialsToday: 55, goalCalls: 100, connected: 35, interested: 9, salesAchieved: 110000, salesTarget: 200000, conversionRate: 16.0, portal: 'telecaller' }
    ];
    for (const m of members) {
      insertTeamMember.run(m.id, m.empCode, m.name, m.avatar, m.role, m.groupName, m.phone, m.attendanceStatus, m.checkInTime, m.checkInMethod, m.dialsToday, m.goalCalls, m.connected, m.interested, m.salesAchieved, m.salesTarget, m.conversionRate, m.portal);
    }
  }
...
export function resetDatabaseToClean() {
  console.log('[SQLite DB] Preserving backup snapshot before reset...');
  createDatabaseBackup('pre-reset');
